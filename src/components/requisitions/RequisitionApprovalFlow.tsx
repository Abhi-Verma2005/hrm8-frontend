import { ApprovalStep } from "@/types/requisition";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Check, X, Clock } from "lucide-react";
import { formatDistanceToNow } from "date-fns";

interface RequisitionApprovalFlowProps {
  approvalWorkflow: ApprovalStep[];
}

export function RequisitionApprovalFlow({ approvalWorkflow }: RequisitionApprovalFlowProps) {
          ...
          {approvalWorkflow.map((step, index) => (
            <div key={step.id} className="flex items-start gap-4">
              ...
              {index < approvalWorkflow.length - 1 && (
                <div className="absolute left-[34px] mt-10 w-0.5 h-16 bg-border" />
              )}
            </div>
          ))}
        </div>
