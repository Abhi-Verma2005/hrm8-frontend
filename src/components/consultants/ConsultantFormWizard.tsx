...
export function ConsultantFormWizard({ consultant, initialRole, onSave, onCancel }: ConsultantFormWizardProps) {
  // Map initialRole to type if provided
  const getInitialType = (): string => {
    if (consultant?.type) return consultant.type;
    if (initialRole === 'SALES_AGENT') return 'sales-rep';
    if (initialRole === 'RECRUITER') return 'recruiter';
    if (initialRole === 'CONSULTANT_360') return '360-consultant';
    return 'recruiter';
  };
  ...
  const handleSave = async () => {
    ...
    // Map type to backend role
    let role: 'RECRUITER' | 'SALES_AGENT' | 'CONSULTANT_360' = 'RECRUITER';
    switch (values.type) {
      case 'sales-rep':
        role = 'SALES_AGENT';
        break;
    ...
    const consultantData: Partial<Consultant> = {
      ...
      photo: values.photo,
      type: values.type,
      role: role, // Include the mapped role
