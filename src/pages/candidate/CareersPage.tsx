...
import { Building2, Globe, Briefcase, Search, ArrowRight, Linkedin, Twitter, SlidersHorizontal, X } from "lucide-react";
  ...
  const loadCompanies = async () => {
    ...
    try {
      const response = await apiClient.get<{
        ...
        total: number;
      }>("/api/public/careers/companies?limit=100");
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {company.social && (
                        <div className="flex gap-1.5">
                          {company.social.linkedin && <Linkedin className="h-4 w-4 text-muted-foreground" />}
                          {company.social.twitter && <Twitter className="h-4 w-4 text-muted-foreground" />}
                        </div>
                      )}
                    </div>
