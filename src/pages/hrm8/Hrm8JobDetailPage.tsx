...
export default function Hrm8JobDetailPage() {
  ...
  const toggleVisibility = async () => {
    if (!job) return;
    try {
      const response = await apiClient.put(`/api/hrm8/jobs/${job.id}/visibility`, { hidden: !job.hrm8Hidden });
  ...
  const changeStatus = async (newStatus: string) => {
    if (!job) return;
    try {
      const response = await apiClient.put(`/api/hrm8/jobs/${job.id}/status`, { status: newStatus, notes });
