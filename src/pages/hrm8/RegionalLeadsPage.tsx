...
export default function RegionalLeadsPage() {
  ...
  const fetchRegions = async () => {
    try {
      ...
      if (response && response.data.regions.length > 0) {
        setRegions(response.data.regions);
        // Default to first region or from URL
        const urlRegionId = searchParams.get('region');
        const defaultRegionId = urlRegionId || (hrm8User?.licenseeId ? response.data.regions.find(r => r.licenseeId === hrm8User.licenseeId)?.id : response.data.regions[0].id);
        if (defaultRegionId) {
